...
class Trainer:
    def __init__(self, args):
        ...
        # MODIFICATION 1: RE-INTRODUCE CLASS WEIGHTS
    ...
    def compute_metrics_binary(self, y_true, y_prob_pos_class, y_pred):
        ...
        if cm.size == 4: # Ensure confusion matrix is 2x2
            tn, fp, fn, tp = cm.ravel()
            sens = tp / (tp + fn + 1e-8)
            spec = tn / (tn + fp + 1e-8)
        else: # Handle cases where only one class is predicted
    ...
    def train(self):
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(train_val_seq), 1):
            ...
            for ep in range(self.args.epochs):
                ...
                # MODIFICATION 2: PRINT CONFUSION MATRIX PER EPOCH
            ...
            # Load the best model from this fold for final fold evaluation
            ...
            print("\n--- Best Model From Fold Validation Metrics (Binary Outcome) ---")
            for k,v in val_metrics_binary.items():
                if k == 'confusion_matrix':
                    print(f"  {k}:\n{v}")
                else:
                    print(f"  {k}: {v:.4f}")
            print("\n--- Best Model From Fold Validation Metrics (CPC Outcome) ---")
            ...
            # Update the overall best model if this fold's best is better
        ...
        for k in fold_metrics_binary[0]:
            if k != 'confusion_matrix':
                mean_val = np.mean([m[k] for m in fold_metrics_binary if not np.isnan(m[k])])
                print(f"  Mean {k}: {mean_val:.4f}")
        print("\n=== Mean CV Validation Metrics (CPC) ===")
        for k in fold_metrics_cpc[0]:
            if k != 'confusion_matrix':
                mean_val = np.mean([m[k] for m in fold_metrics_cpc if not np.isnan(m[k])])
                print(f"  Mean {k}: {mean_val:.4f}")
