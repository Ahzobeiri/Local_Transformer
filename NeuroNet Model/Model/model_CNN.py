...
class ChannelProcessor(nn.Module):
    def __init__(self, in_channels: int = 19, hidden_channels: int = 64):
        super().__init__()
        # CNN approach for channel fusion
        ...
        self.conv3 = nn.Conv1d(hidden_channels // 2, 1, kernel_size=3, padding=1)
        
        # Optional: learnable channel weights for weighted averaging
        self.channel_weights = nn.Parameter(torch.ones(in_channels) / in_channels)
        self.use_weighted_avg = False  # Set to True to use weighted averaging instead of CNN
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_weighted_avg:
            # Alternative: weighted average across channels
...
if __name__ == '__main__':
    ...
    # Multi-channel test
    x_multi = torch.randn((50, 19, 3000))  # Your multi-channel input
    channel_processor = ChannelProcessor(in_channels=19)
    x_processed = channel_processor(x_multi)  # Convert to (50, 3000)
    
    # Now use the processed data with NeuroNet
    m0.forward(x_processed, mask_ratio=0.5)
