...
class MAMBA_TCM(TemporalContextModule):
    ...
    def forward(self, x):
        ...
        # Apply backbone and Mamba
        x_embedded = self.apply_backbone(x)
        mamba_out = self.mamba(x_embedded)  # (batch, seq_len, embed_dim)
        
        # Project Mamba output
        mamba_projected = self.fc_project(mamba_out)  # (batch, seq_len, 30)
        
        # Extract features for ALL timesteps
        all_features = []
        for t in range(seq_len):
            snippet = x[:, t, :]
            features = extract_eeg_features(snippet, self.sfreq).to(x.device)
